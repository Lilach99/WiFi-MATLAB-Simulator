function [] = simulateDifferentSTsBunchInDataRate(dataRate, packetsPolicy)
    ...
    for k=1:numSims
        ...
        %parfor h=1:numDists
          ...
          %mkdir(resPath);
          outputStandard{h} = simulateNet(9*10^-6, simTime, numDevs, 0, 0, h, dataRate, packetsPolicy, resPath); % the last parameter is dataRate in Mbps!
          link1InfoStandardST{h} = outputStandard{h}.linksRes{1};
          link2InfoStandardST{h} = outputStandard{h}.linksRes{2};

          disp('2APD');
          output2APD{h} = simulateNet(2*ST, simTime, numDevs, 0, 0, h, dataRate, packetsPolicy, resPath);
          link1Info2APDST{h} = output2APD{h}.linksRes{1};
          link2Info2APDST{h} = output2APD{h}.linksRes{2};

          disp('3APD');
          output3APD{h} = simulateNet(3*ST, simTime, numDevs, 0, 0, h, dataRate, packetsPolicy, resPath);
          link1Info3APDST{h} = output3APD{h}.linksRes{1};
          link2Info3APDST{h} = output3APD{h}.linksRes{2};

          disp('APD');
          outputAPD{h} = simulateNet(ST, simTime, numDevs, 0, 0, h, dataRate, packetsPolicy, resPath);
          link1InfoAPDST{h} = outputAPD{h}.linksRes{1};
          link2InfoAPDST{h} = outputAPD{h}.linksRes{2};

          disp('0.5APD');
          outputHalfAPD{h} = simulateNet(0.5*ST, simTime, numDevs, 0, 0, h, dataRate, packetsPolicy, resPath);
          link1InfoHalfAPDST{h} = outputHalfAPD{h}.linksRes{1};
          link2InfoHalfAPDST{h} = outputHalfAPD{h}.linksRes{2};

          disp('0.25APD');
          outputQrtAPD{h} = simulateNet(0.25*ST, simTime, numDevs, 0, 0, h, dataRate, packetsPolicy, resPath);
