function [updatedList] = updateEventsList(newEvents, curTime, curEvents)
    ...
    if(size(newEvents, 2) ~= 0)
        ...
        updatedList = sortByHandlingOrder([curEvents, curTimeEvents]); % add new 'curTime' events to the list, and sort according to the handling order
    else 
        % no new events
        updatedList = sortByHandlingOrder(curEvents);
    end
end
